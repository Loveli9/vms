...
//  var mode = "popup"//编辑框的模式：支持popup和inline两种模式，默认是popup
    ...
    /*function initTableSave(){
        ...
        var url = getRootPath() + '/projectReport/getProjectDscPageByNo';
...
//    	height:tableHeight(),//高度调整
        ...
        columns: [
            {title:'全选',field:'select',checkbox:true,width:25},
            {
        		title : '序号',
        		field: 'num',
        		align: "center",
...
//                    mode: mode,
    ...
    /*function initProMember() {//默认加载项目成员下拉列表
    	var options=getMeberSelect();
    	var select_option="";
		$("#personLiableAdd").empty();
		_.each(options, function(val, index){//(值，下标)
			select_option += "<option value="+options[index].value+">"+options[index].text+"</option>";
		})
		$("#personLiableAdd").html(select_option);
	};	*/
    ...
    $('#btn_edit').click(function(){
        ...
        var selectRow = $('#mytable').bootstrapTable('getSelections');
        initEditForm();
        ...
        $("#idEdit").val(selectRow[0].id);
        $("#versionEdit").val(selectRow[0].version);
        $("#proNoEdit").val(selectRow[0].no);
        $("#questionEdit").val(selectRow[0].question);
        $("#imprMeasureEdit").val(selectRow[0].imprMeasure);
        $("#progressDescEdit").val(selectRow[0].progressDesc);
        $("#finishTimeEdit").val( new Date(selectRow[0].finishTime).format('yyyy-MM-dd'));
        if(selectRow[0].actualFinishTime){
        	$("#actualFinishTimeEdit").val( new Date(selectRow[0].actualFinishTime).format('yyyy-MM-dd'));
        }else{
        	$("#actualFinishTimeEdit").val('');
        }
        $("#is361Edit").val(selectRow[0].is361);
        $("#stateEdit").val(selectRow[0].state);
        
        $("#topicEdit").val(selectRow[0].topic);
        $("#probabilityEdit").val(selectRow[0].probability);
        $("#speedEdit").val(selectRow[0].speed);
        
        $("#severityEdit").val(selectRow[0].severity);
        $("#priorEdit").val(selectRow[0].prior);
        $("#iteIdEdit").val(selectRow[0].iteId);
        /*document.getElementById('severityEdit').value = selectRow[0].severity;
        document.getElementById('priorEdit').value = selectRow[0].prior;
        document.getElementById('iteIdEdit').value = selectRow[0].iteId;*/
        //处理责任人
        if(selectRow[0].personLiable){
        	var option = (selectRow[0].personLiable).split(",");
