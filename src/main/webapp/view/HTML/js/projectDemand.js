...
$(function () {
    ...
    function initEditForm(member) {
        ...
        setEditMeberOption('editPersonLiable', member);
    ...
    $('#mytab').bootstrapTable({
        ...
        url: getRootPath() + '/IteManage/queryAll',
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    var tab = '<div style="width: 100%;text-align: left;margin-left:10px;">' +
                        '<div style="width: 100%;">' +
                        '<a href="#" title=' + value + ' style="font-size: 14px;color:blue" onclick="editData(\'' + row.id + '\',\'' + row.proNo + '\')">' + value + '</a>' +
                        '</div>' +
...
//        		visible: false,
            ...
            },
            {
                ...
                width: '8%',
                formatter: function (value, row, index) {
                    // return '<button id="edit" type="button" class="btn edit btn-sm" style=" border-radius: 0">' +
                    //     '<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>修改' +
                    //     '</button>' +
                    //     '<button id="delete" type="button" class="btn delete btn-sm">' +
                ...
                events: {
                    ...
                    'click a[title=修改]': function (e, value, row, index) {
                        ...
                        var member = getMebersSelect(row.proNo);
                        initEditForm(member);
    ...
    $('#search_btn').click(function () {
        // demand_list = "all";
        $('#mytab').bootstrapTable('refresh', {url: getRootPath() + '/IteManage/queryAll'});
    });
...
//                $("#editUpdateTime").val(changeDateFormatHMS(data.rows.updateTime));
                    ...
                    success: function (data) {
                        if (data.code == '200') {
                            ...
                            if (demand_list == "all") {
                                ...
                                $('#mytab').bootstrapTable('refresh', {url: getRootPath() + '/IteManage/queryAll?type=ready'});
                            }
                        } else {
    ...
    //点击标题启用编辑
    editData = function (selectId, proNo) {
        $("#editIteId").empty();
        $("#editWrField").empty();
        $("#editPrior").empty();
        $("#editImportance").empty();
        ...
        $("#editChange").empty();
        $("#editPersonLiable").empty();
        var member = getMebersSelect(proNo);
        initEditForm(member);
        $("#editPage").modal('show');
        $.ajax({
            url: getRootPath() + '/IteManage/editPage',
    ...
    function deleteData(id) {
        ...
        Ewin.confirm({message: "确认删除当前工作内容吗？"}).on(function (e) {
            if (!e) {
                ...
                $.ajax({
                    url: getRootPath() + '/IteManage/deleteIte',
                    type: 'post',
                    dataType: "json",
                    contentType: 'application/json;charset=utf-8', //设置请求头信息
    ...
    $("#btn_export").click(function () {
        window.location.href = getRootPath() + "/IteManage/exportNew";
    })
    ...
    function getMebersSelect(proNo) {
        var s1 = "";
        $.ajax({
...
//项目成员列表加载
    ...
    //项目成员列表加载
    function setEditMeberOption(id, member) {
        $('#editPersonLiable').selectpicker("val", []);
        $("#editPersonLiable").empty();
        $('#editPersonLiable').prev('div.dropdown-menu').find('ul').empty();
        for (var i = 0; i < member.length; i++) {
            $("#editPersonLiable").append("<option value='" + member[i].value + "'>" + member[i].text + "</option>");
        }
        $('#editPersonLiable').selectpicker('refresh');
        $('#editPersonLiable').selectpicker('render');
    }
