$(function(){
	var projectState = "在行";
	var myCharts1=echarts.init(document.getElementById("kaifaguochengZL"));
	var pcbCategory = "开发过程质量";
	var measureIds = ["239","240","241","242"];
	...
	$.fn.loadkaifaGuochengXL=function(){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#tablekaifaguochengZL tr").remove();
				var tab = "";
				var deng = "";
				_.each(data.data,function(lists,index){
					_.each(lists,function(list,index1){
						...
						if(list.name=="代码检视缺陷密度"){
							val1[index1]=list.num;
						}else if(list.name=="SDV测试缺陷密度"){
							val2[index1]=list.num;
						}else if(list.name=="SDV自动化用例比率"){
							val3[index1]=list.num;
						}else if(list.name=="软件开发生产率"){
							val4[index1]=list.num;
						}
					});
				});
				
				$("#tablekaifaguochengZL").append(tab);
				chartskaifaGuochengXL(types,month,val1,val2,val3,val4);
			}
		});
		
		$('#tablekaifaguochengXM').bootstrapTable({
			method: 'get',
	...
	function chartskaifaGuochengXL(types,month,val1,val2,val3,val4){
		//折线图
		var options1={
				//定义标题
				title:{
		...
		$('#tableProject').bootstrapTable({
			...
			columns:[
	            ...
	            },
	            {
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id239',value);
	    			}
	            },
				{
	        		title : 'SDV测试缺陷密度',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id240',value);
	    			}
	            },
				{
	        		title : 'SDV自动化用例比率',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id241',value);
	    			}
	            },
				{
	        		title : '软件开发生产率',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id242',value);
	    			}
	            }
	        ],
			locale:'zh-CN'//中文支持
...
function selectOnchange(){
	 $().loadkaifaGuochengXL();
	 $('#tablekaifaguochengXM').bootstrapTable('refresh');
}