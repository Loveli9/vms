...
/*	var area="";
	...
	var du="";
*/	var projectState = "在行";
	var myCharts1=echarts.init(document.getElementById("ceshiGuochengZL"));
	var pcbCategory = "测试过程质量";
	var measureIds = ["245","246"];
	/*function menuChange(){
	...
	$.fn.loadCeshiGuochengXL=function(){
        var clientType = "1";
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#tableCeshiGuochengZL tr").remove();
				var tab = "";
				var deng = "";
				_.each(data.data,function(lists,index){
					_.each(lists,function(list,index1){
						...
						if(list.name=="用例发现的问题占比"){
							valLow[index1]=list.num;
						}else if(list.name=="自动化用例占比"){
							valNo[index1]=list.num;
						}
					});
				});
				
				$("#tableCeshiGuochengZL").append(tab);
				chartsCeshiGuochengXL(types,month,valLow,valNo);
			}
		});
		$('#tableCeshiGuochengXM').bootstrapTable({
			method: 'get',
	...
	function chartsCeshiGuochengXL(types,month,valLow,valNo){
		//折线图
		var options1={
				//定义标题
				title:{
		...
		$('#tableProject').bootstrapTable({
			...
			columns:[
	            ...
	            },
	            {
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id245',value);
	    			}
	            },
				{
	        		title : '自动化用例占比',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id246',value);
	    			}
	            }
	        ],
			locale:'zh-CN'//中文支持
...
function selectOnchange(){
	 $().loadCeshiGuochengXL();
	 $('#tableCeshiGuochengXM').bootstrapTable('refresh');
}