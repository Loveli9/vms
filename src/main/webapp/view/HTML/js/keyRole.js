			...
			success : function(data) {
				for(var i=0;i<data.length;i++){
					var tab = '<tr style="height: 52px;">'+
						...
						'<td date-type="input" name="staffname">'+data[i].name+'</td>'+
						'<td date-type="input" name="zrAccount">'+(data[i].zrAccount=="null"?"NA":data[i].zrAccount)+'</td>'+
						'<td date-type="input" name="hwAccount">'+(data[i].hwAccount=="null"?"NA":data[i].hwAccount)+'</td>'+
						'<td date-type="select" select-date="未通过,通过,不涉及">'+(data[i].rdpmExam=="null"?"NA":data[i].rdpmExam)+'</td>'+
						'<td date-type="select" select-date="未通过,通过,不涉及">'+data[i].replyResults+'</td>'+
