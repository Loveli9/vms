$(function(){
    var projectState = "在行";
    var myCharts1=echarts.init(document.getElementById("ceshiXL"));
    var pcbCategory = "迭代效率";
    var measureIds = ["254","255","256","257","258","259"];
    var nos = "";
    function exportbut(){
		$("#exportbut").click(function(){
            ...
            $eleForm.attr("action",getRootPath() + "/pcbList/exportIteration");
	...
	$.fn.loadkaifaGuochengXL=function(){
		...
		$.ajax({
			url: getRootPath() + '/pcbList/getPcbIterativeEfficiency',
			type: 'post',
			...
			success: function (data) {
				...
				_.each(data.data,function(lists,index){
					_.each(lists,function(list,index1){
						...
						if(list.name=="java迭代生产率"){
							valLow[index1]=list.num;
						}else if(list.name=="C/C++迭代生产率"){
							valNo[index1]=list.num;
						}else if(list.name=="迭代测试用例设计效率"){
							val1[index1]=list.num;
						}else if(list.name=="迭代测试用例执行效率"){
							val2[index1]=list.num;
						}else if(list.name=="自动化用例写作效率"){
							val3[index1]=list.num;
						}else if(list.name=="自动化脚本连跑通过率"){
							val4[index1]=list.num;
						}
					});
				});
				
				$("#tableCeshiXL").append(tab);
				chartsCeshiXL(types,month,valLow,valNo,val1,val2,val3,val4);
			}
		});
		//详情页面
		$("#tableCeshiXM").bootstrapTable({
			method: 'get',
	    	contentType: "application/x-www-form-urlencoded",
	    	url: getRootPath() + '/pcbList/getPcbIterativeEfficiencyXM',
	...
	function chartsCeshiXL(types,month,valLow,valNo,val1,val2,val3,val4){
		//折线图
		var options1={
				//定义标题
				title:{
...
function saveChangeTar(val){
	...
	if(!c){
		...
		$.ajax({
            type: "post",
            url: getRootPath()+"/measure/updateMeasure",
			dataType: "json",
			async: false,
...
function selectOnchange(){
	 $().loadkaifaGuochengXL();
	 $('#tableCeshiXM').bootstrapTable('refresh');
}