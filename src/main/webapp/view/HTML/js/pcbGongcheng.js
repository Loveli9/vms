...
/*	var area="";
	...
	var du="";
*/	var projectState = "在行";
	var myCharts1=echarts.init(document.getElementById("gongchengNL"));
	var pcbCategory = "工程能力";	
	var measureIds = ["229","230","231","232","233","234","235","236"]; 
	/*function menuChange(){
			...
			success: function (data) {
				...
				_.each(data.data,function(lists,index){
					_.each(lists,function(list,index1){
						...
						}else if(list.name=="版本每日可用率"){
							val2[index1]=list.num;
						}else if(list.name=="个人构建平均时长"){
							val3[index1]=list.num;
						}else if(list.name=="版本级编译构建时间"){
							val4[index1]=list.num;
						}else if(list.name=="全量自动化回归验证周期"){
							val5[index1]=list.num;
						}else if(list.name=="全量功能验证周期"){
							val6[index1]=list.num;
						}else if(list.name=="解决方案验证周期"){
							val7[index1]=list.num;
						}else if(list.name=="特性交付周期CycleTime"){
							val8[index1]=list.num;
						}
					});
				});
				
				$("#tableGongchengNL").append(tab);
				chartsCeshiGuochengXL(types,month,val1,val2,val3,val4,val5,val6,val7,val8);
			}
		});
		$('#tableGongchengNLXM').bootstrapTable({
			method: 'get',
	...
	function chartsCeshiGuochengXL(types,month,val1,val2,val3,val4,val5,val6,val7,val8){
		//折线图
		var options1={
				//定义标题
				title:{
		...
		$('#tableProject').bootstrapTable({
			...
			columns:[
	            ...
	            },
	            {
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id229',value);
	    			}
	            },
				{
	        		title : '版本每日可用率',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id230',value);
	    			}
	            },
				{
	        		title : '个人构建平均时长',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id231',value);
	    			}
	            },
				{
	        		title : '版本级编译构建时间',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id232',value);
	    			}
	            },
				{
	        		title : '全量自动化回归验证周期',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id233',value);
	    			}
	            },
	            {
	        		title : '全量功能验证周期',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id234',value);
	    			}
	            },
	            {
	        		title : '解决方案验证周期',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id235',value);
	    			}
	            },
	            {
	        		title : '特性交付周期CycleTime',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id236',value);
	    			}
	            }
	        ],
			locale:'zh-CN'//中文支持
...
function selectOnchange(){
	 $().loadGongchengNL();
	 $('#tableGongchengNLXM').bootstrapTable('refresh');
}
