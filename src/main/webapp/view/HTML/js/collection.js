...
//	var projNo = window.parent.projNo;
	...
	$(document).ready(function(){
		$.ajax({
	        ...
	        data:{
	        	projectId : window.parent.projNo
	        },
			success: function (data) {
				$("#name").append(data);
	        	$("#name").attr("title",data);
			}
		});
	});
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            _.each(data, function(config, index){
              ...
              if(config.types){
                ...
                for(var i=0;i<types.length;i++){
                  ...
                  if(types[i]=="svn"){
                    ...
                    msg="SmartIDE";
                  }else if(types[i]=="jenkins"){
                    msg = "jenkins";
                  }else if(types[i]=="isource"){
                    msg="ISOURCE";
                  }else if(types[i]=="clouddragon"){
                    msg="CLOUDDRAGON";
                  }
              ...
              var date = new Date(config.modifyTime);
              ...
              tab+='<div name="collectionJobModu" class="divgezi">'+
                '<a id="startCollect" jobid="'+config.id+'" jobtypes="'+config.types+'" jobids="'+config.ids+'" href="#" style="width: auto;height: auto;">'+
                  '<div style="height: 112px;color: #b1b1b1;">'+
                    '<div class="JobName">'+configname+'<div class="sign sign-'+color+'"></div></div>'+
                    '<div style="margin-left: 20px;">最近采集时间：'+new Date(config.modifyTime).format('yyyy-MM-dd hh:mm:ss')+'</div>'+
                    '<div class="cjwcd">'+
    ...
    $(document).on("click", "#returnStep1", function () {
    	...
    	$('#tabsTitleOne', parent.document).html("采集配置任务");
    });
		...
		$.ajax({
			...
			success: function (data) {
				_.each(data, function(config, index){
					...
					if (config.types.indexOf("jenkins") != -1) {
                        msg += "jenkins" + "/";
                    }
					if(config.types.indexOf("isource")!=-1){
						msg += "ISOURCE"+"/";
					}
					if(config.types.indexOf("clouddragon")!=-1){
						msg += "CLOUDDRAGON"+"/";
					}
					if(msg.indexOf("/")){
					...
					$("#collectionType1").html(configname);
					$('#tabsTitleOne', parent.document).html(configname);
...
//		var types = "";
		...
		_.each($("div[name=isSelect]"), function(config, index){
			...
			if(tabname=='svn'){
				...
				selectedIds += '6';
			}else if(tabname=='isource') {
				selectedIds += '8';
			}else if(tabname=='jenkins') {
				selectedIds += '9';
			}else if(tabname=='clouddragon') {
				selectedIds += '7';
			}
		});
		...
		_.each($('#isourceTable tbody tr'), function(tr, index){
			repositoryInfos.push({
				type:8,
				id:$(tr.cells[0].children[0]).attr("value"),
				url:tr.cells[0].children[0].innerHTML,
		...
		_.each($('#jenkinsTable tbody tr'), function(tr, index){
			repositoryInfos.push({
				type:9,
				id:$(tr.cells[0].children[0]).attr("value"),
				url:tr.cells[0].children[0].innerHTML,
		...
		_.each($('#clouddragonTable tbody tr'), function(tr, index){
			repositoryInfos.push({
				type:7,
				id:$(tr.cells[0].children[0]).attr("value"),
				url:tr.cells[0].children[0].innerHTML,
...
//		var len = $("span[name=del]").length;
//		var myMap=new Map();
//		_.each($("div[name=isSelect]"), function(config, index){
//			var tabname = $(config).parent("li").attr("tabname");
//			tabname = tabname.substring(4);
//			myMap.set(tabname,$(config).attr("isSelectFlag"));
//		});
//		_.each($("span[name=del]"), function(config, index){
//			if(len==(index+1)){
//				ids += $(config).parent('td').attr("value");
//			}else{
//				ids += $(config).parent('td').attr("value")+";";
//			}
//			if(myMap.get($(config).attr("tabid"))!="0"){
//				return true;
//			}
//			selectedIds += $(config).parent('td').attr("value")+";";
//			types += $(config).attr("tabid")+";";
//		});
...
//			info();
	...
	//新建任务的采集返回
	$(document).on("click", "#retTop", function () {
		...
		$("#jenkinsTable tbody tr").remove();
		$("#isourceTable tbody tr").remove();
		$("#clouddragonTable tbody tr").remove();
	...
	$(document).on("click", "a[id^='editJob']", function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				_.each(data, function(config, index){
					...
					//1:DTS,2:SVN,3:Git,4:TMSS,5:ci,6:smartIDE
					if(configname=="1"){
						dtsAddDo(config,"dts");
					...
					}else if(configname=="8"){
						isourceAddDo(config,"isource");
					}else if(configname=="9"){
                    	jenkinsAddDo(config,"jenkins");
                    }else if(configname=="7"){
                    	clouddragonAddDo(config,"clouddragon");
                    }  
				});
				hasDate("dts");
				hasDate("svn");
    	...
    	$('#tabsTitleOne', parent.document).html("新建任务");
	});
	...
	$(document).on("click", "span[name=del]", function () {
		...
		var table = $(this).parent('td').parent('tr').parent('tbody').parent('table').attr("id");
		var del = this;
		Ewin.confirm({ message: "此操作会删除当前配置的采集数据，确认执行吗？" }).on(function (e) {
			...
			$.ajax({
				...
				success: function (data) {
					if(table=="jenkinsTable"){
						$("#tab-jenkins #addData").removeAttr("disabled");
						$("#tab-jenkins #addData").css("background-color","#e41f2b");
						$("#tab-jenkins #addData").css("border-color","#e41f2b");
					}
				}
			});
...
//			$("#tab-jenkins #addData").attr("disabled","disabled");
//			$("#tab-jenkins #addData").css("background-color","#999999");
//			$("#tab-jenkins #addData").css("border-color","#999999");
			flag = jenkinsAdd(data_id);
		}else if(id=="isource"){
			flag = isourceAdd(data_id);
		}else if(id=="clouddragon"){
			flag = clouddragonAdd(data_id);
		}
	...
	function editTr(configname,tds){
		...
		}else if(configname=="isource"){
			$('#'+configname+'Version').val($($(tds[1]).children("span")[0]).attr("title"));
		}else if(configname=="jenkins"){
			$('#jenkinsurl').val($($(tds[0]).children("span")[0]).attr("title"));
			$('#jenkinsVersion').val($($(tds[1]).children("span")[0]).attr("title"));
			editOtherAcc(configname,$($(tds[2]).children("span")[0]).attr("value"));
		}else if(configname=="clouddragon"){
			$('#clouddragonVersion').val($($(tds[1]).children("span")[0]).attr("title"));
			$('#clouddragonPath').val($($(tds[2]).children("span")[0]).attr("title"));
		}
	}
	...
	function isourceAddDo(config,counter){
		var tab = "";
		tab+='<tr>'+
				'<td><span value="'+config.id+'" title="'+valueIsNull(config.url)+'">'+valueIsNull(config.url)+'</span></td>'+
				'<td><span title="'+valueIsNull(config.branch)+'">'+valueIsNull(config.branch)+'</span></td>';
		tab+='<td value="'+config.id+'">'+
			'<span name="edit" tabid="isource" class="glyphicon glyphicon-edit glyphiconNew" aria-hidden="true"></span>'+
			'<span name="del" tabid="isource" class="glyphicon glyphicon-trash glyphiconNew" aria-hidden="true"></span></td>'+
			"</tr>";
		$("#isourceTable tbody").append(tab);
	}
	function jenkinsAddDo(config, counter) {
		var tab = "";
		tab+='<tr>'+
				'<td><span value="'+config.id+'" title="'+valueIsNull(config.url)+'">'+valueIsNull(config.url)+'</span></td>'+
				'<td><span title="'+valueIsNull(config.branch)+'">'+valueIsNull(config.branch)+'</span></td>';
		...
		tab+='<td value="'+config.id+'">'+
			'<span name="edit" tabid="jenkins" style="cursor: pointer;" class="glyphicon glyphicon-edit glyphiconNew" aria-hidden="true"></span>'+
			'<span name="del" tabid="jenkins" style="cursor: pointer;" class="glyphicon glyphicon-trash glyphiconNew" aria-hidden="true"></span></td>'+
			"</tr>";
		$("#jenkinsTable tbody").append(tab);
//		if($("#jenkinsTable tbody").children('tr').length==1){
			$("#tab-jenkins #addData").attr("disabled","disabled");
			$("#tab-jenkins #addData").css("background-color","#999999");
			$("#tab-jenkins #addData").css("border-color","#999999");
//		}
	...
	function clouddragonAddDo(config,counter){
		var tab = "";
		tab+='<tr>'+
				'<td><span value="'+config.id+'" title="'+valueIsNull(config.url)+'">'+valueIsNull(config.url)+'</span></td>'+
				'<td><span title="'+valueIsNull(config.excludePath)+'">'+valueIsNull(config.excludePath)+'</span></td>'+
				'<td><span title="'+valueIsNull(config.excludeRevision)+'">'+valueIsNull(config.excludeRevision)+'</span></td>';
		tab+='<td value="'+config.id+'">'+
			'<span name="edit" tabid="clouddragon" class="glyphicon glyphicon-edit glyphiconNew" aria-hidden="true"></span>'+
			'<span name="del" tabid="clouddragon" class="glyphicon glyphicon-trash glyphiconNew" aria-hidden="true"></span></td>'+
			"</tr>";
		$("#clouddragonTable tbody").append(tab);
	}
...
//		});
	...
	function tmssAdd(id){
		...
		tmssAddDo(config,"tmss");
//		allTr = getRequestJson(id,allTr,4,tmssurls,tmssVersions,null,tmssOtherAccs);
//		
//		var jsonStr = '{"projNo":"'+projNo+'","repositoryInfos":['+allTr.substring(0,allTr.length-1)+']}';
//		$.ajax({
//			url: getRootPath() + '/svnTask/saveurl1',
...
//		});
	...
	function icpCiAdd(id){
		...
		icpCiAddDo(config,"icp-ci");
//		allTr = getRequestJson(id,allTr,5,ciurls,null,null,null,icpPaths,icpVersions);
//		
//		var jsonStr = '{"projNo":"'+projNo+'","repositoryInfos":['+allTr.substring(0,allTr.length-1)+']}';
//		$.ajax({
//			url: getRootPath() + '/svnTask/saveurl1',
...
//		});
	...
	// 保存jenkins配置
    function jenkinsAdd(id) {
    	var allTr = "";
		var jenkinsurl = $('#jenkinsurl');
		var jenkinsVersion = $('#jenkinsVersion');
		var jenkinsOtherAcc = $('#jenkinsOtherAcc');
		var flag=false;
		_.each($('#jenkinsTable tbody tr'), function(tr, index){
			if(jenkinsurl[0].value==tr.cells[0].children[0].innerHTML && 
					jenkinsVersion[0].value==tr.cells[1].children[0].innerHTML ){
				flag=true;
				return false;
			};
		});
		...
		var config = {
				id:id?id:'',
				url:jenkinsurl[0].value,
				branch:jenkinsVersion[0].value,
				otherAccount:$(jenkinsOtherAcc[0]).prop("checked")?'0':'1'
		}
		jenkinsAddDo(config, "jenkins");
    }
	function isourceAdd(id){
		var allTr = "";
		var isourceurls = $('#isourceurlNew');
		var isourceVersions = $('#isourceVersion');//
		
		var flag=false;
		_.each($('#isourceTable tbody tr'), function(tr, index){
			if(isourceurls[0].value==tr.cells[0].children[0].innerHTML && 
					isourceVersions[0].value==tr.cells[1].children[0].innerHTML ){
				flag=true;
				return false;
			};
		});
		...
		var config = {
				id:id?id:'',
				url:isourceurls[0].value,
				branch:isourceVersions[0].value,
		}
		isourceAddDo(config,"isource");
	}
	
	function clouddragonAdd(id){
		var allTr = "";
		var clouddragonurls = $('#clouddragonurlNew');
		var clouddragonVersions = $('#clouddragonVersion');//版本过滤条件
		var clouddragonPaths = $('#clouddragonPath');//路径过滤条件
		
		var flag=false;
		_.each($('#clouddragonTable tbody tr'), function(tr, index){
			if(clouddragonurls[0].value==tr.cells[0].children[0].innerHTML && 
					clouddragonVersions[0].value==tr.cells[1].children[0].innerHTML && 
					clouddragonPaths[0].value==tr.cells[2].children[0].innerHTML ){
				flag=true;
				return false;
			};
		});
		...
		var config = {
				id:id?id:'',
				url:clouddragonurls[0].value,
				excludeRevision:clouddragonPaths[0].value,
				excludePath:clouddragonVersions[0].value,
		}
		clouddragonAddDo(config,"clouddragon");
	}
	
	$(document).on("click", "#collectionStart", function () {
		jobids = $(this).attr("jobids");
		$.ajax({
			url: getRootPath() + '/project/searchConfigInfoByJob',
			type: 'post',
			data: {
			...
			success: function (data) {
				...
				_.each(data, function(config, index){
					...
					if(configname=="1"){
						...
						logmessage('',new Date().format('yyyy-MM-dd hh:mm:ss')+'  isource采集('+msg+")",'Start');
						$.ajax({
							url: getRootPath() + '/acquire/isourceIssue',
							type: 'post',
							data: {
								no: projNo,
								token : token,
					...
					}else if (configname == "9") {// jenkins数据采集
						logmessage('',new Date().format('yyyy-MM-dd hh:mm:ss')+'  jenkins采集('+msg+")",'Start');
						$.ajax({
							url: getRootPath() + '/jenkins/jenkinsCollect',
							type: 'post',
							data: {
								no: projNo,
								token : token,
								id: config.id, //项目配置ID
								url : config.url,
								jobName : config.branch,
								otherAccount : config.otherAccount
							},
							success:function(data){
//								haveExcute +=1;
//								if(data.code == 'success'){//成功标识
...
//								}
                    ...
                    }else if (configname == "7") {// clouddragon数据采集
						logmessage('',new Date().format('yyyy-MM-dd hh:mm:ss')+'  clouddragon采集('+msg+")",'Start');
						$.ajax({
							url: getRootPath() + '/acquire/clouddragonCollect',
							type: 'post',
							data: {
								no: projNo,
								token : token,
