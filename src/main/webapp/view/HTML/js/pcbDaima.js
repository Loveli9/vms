var projectState = "在行";
var myCharts1;
var pcbCategory = "代码质量";	
var measureIds = ["223","224","225","226","227","228","247","248"];
$(function(){
		...
		$('#tableProject').bootstrapTable({
			...
			columns:[
	            ...
	            },
	            {
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id223',value);
	    			}
	            },
				{
	        		title : '代码重复率',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id224',value);
	    			}
	            },
				{
	        		title : '平均圈复杂度',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id225',value);
	    			}
	            },
				{
	        		title : 'Coverity高、中告警数',
	        		field : 'id226',
	        		align : "center",
	        		width : 180,
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id226',value);
	    			}
	            },
				{
	        		title : 'Fortify高、中告警数',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id227',value);
	    			}
	            },
	            {
	        		title : 'SAI（当前值）',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id228',value);
	    			}
	            },
				{
	        		title : '函数圈复杂度违约 ',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id247',value);
	    			}
	            },
	            {
	        		title : '新增危险函数',
	        		...
	        		valign:'middle',
	        		formatter: function (value,row,index){
	        			return formatterColumn('id248',value);
	    			}
	            }
	        ],
			locale:'zh-CN'//中文支持
	...
	$(document).ready(function(){
		myCharts1=echarts.init(document.getElementById("guochengZL"));
		getPcbDaiMa();
...
function selectOnchange(){
	getPcbDaiMa();
	$('#tableGuochengXM').bootstrapTable('refresh');
}


function getPcbDaiMa(){
    var clientType = "1";
		...
		success: function (data) {
			...
			_.each(data.data,function(lists,index){
				_.each(lists,function(list,index1){
					...
					type[index]=list.name;
					month[index1]=list.month;
					if(list.name=="静态检查告警数"){
						val1[index1]=list.num;
					}else if(list.name=="代码重复率"){
						val2[index1]=list.num;
					}else if(list.name=="平均圈复杂度"){
						val3[index1]=list.num;
					}else if(list.name=="Coverity高、中告警数"){
						val4[index1]=list.num;
					}else if(list.name=="Fortify高、中告警数"){
						val5[index1]=list.num;
					}else if(list.name=="SAI（当前值）"){
						val6[index1]=list.num;
					}else if(list.name=="函数圈复杂度违约"){
						val7[index1]=list.num;
					}else if(list.name=="新增危险函数"){
						val8[index1]=list.num;
					}
					
				});
			});
			
			$("#tableGuochengZL").append(tab);
			chartsGuochengZL(type,month,val1,val2,val3,val4,val5,val6,val7,val8);
		}
	});
	$('#tableGuochengXM').bootstrapTable({
		method: 'get',
    	contentType: "application/x-www-form-urlencoded",
...
function chartsGuochengZL(types,month,val1,val2,val3,val4,val5,val6,val7,val8){
	//折线图
	var options1={
			//定义标题
			title:{
